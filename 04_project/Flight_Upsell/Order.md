...
--   - Date range narrowed to 2026-01-01 to 2026-05-31 (aligns with JN)
--   - flightclass = 'I' filter added (international orders only, from JN)
--   - datachange_createtime cutoff updated to 2025-12-01 (from JN)
...
CREATE OR REPLACE TABLE `trip-ibu-adhoc.ibu_adhoc_temp.MZ_202606_quantification_v2`
AS
WITH
  brand_en_name AS (
    SELECT DISTINCT
  ...
  xprod AS (
    ...
    WHERE
      DATE(x.orderdate) BETWEEN "2026-01-01" AND "2026-05-31"
      AND x.sequence = 1
    GROUP BY 1
  ),
  ...
  lowest_price_ranked AS (
    ...
    WHERE
      CAST(d AS DATE) BETWEEN "2026-01-01" AND "2026-05-31"
      AND is_trip = 'T'
  ...
  f_textdetail AS (
    ...
    JOIN (
      ...
      WHERE orderdate_d BETWEEN "2026-01-01" AND "2026-05-31"
  ...
  e_customer_cancellation AS (
    -- From JN: captures service_fee_type (structure of the cancellation fee)
    SELECT DISTINCT
      e.token,
      e.service_fee_type
  ...
  segment_table AS (
    ...
    WHERE
      a.orderdate_d BETWEEN "2026-01-01" AND "2026-05-31"
...
SELECT
  ...
  -- Void / 24hr ticket policy counts
...
LEFT JOIN brand_en_name b
  ON  a.order_marketing_airline = b.carrier
