...
## 3. AI Context 基础设施（已就绪）

| 文件 | 用途 | AI 何时读 |
|------|------|----------|
| `PROJECT_CONTEXT.md`（本文件）| Priority、工作流状态、Next Steps | 每次 session 必读 |
| `CONTEXT.md` | BQ 表结构、字段定义、metric 定义、常见查询模式、已知坑 | 写 SQL 前必读 |
| `upsell_diagnostic_framework.md` | 五层漏斗、两类 needle mover、KPI 可比性 | 做航司诊断或框架级分析时读 |
| `FBU_Mockup_Feedback_Template.md` | FBU/Region 沟通模板（复用框架 + refund PM 联系模板） | 需要起草跨 BU 沟通时读 |
| `CLAUDE.md` | Working Agreement、输出规范、文件地图 | 每次 session 自动加载 |

---

## 4. Five-Layer Diagnostic Framework（结构化贡献）
...
- [ ] 与 refund/change mid-tier PM 建立联系（通过介绍人），确认 scope
...
| 文件 | 用途 |
|------|------|
| `PROJECT_CONTEXT.md`（本文件）| 统领性上下文，每次 session 必读 |
| `CONTEXT.md` | 数据定义、BQ 表结构、metric 定义、常见查询模式、已知坑 |
| `upsell_diagnostic_framework.md` | 五层漏斗详细定义，两类 needle mover，KPI 可比性原则 |
| `void_policy_display_analysis.md` | Void/24h 完整技术 brief |
| `FBU_Mockup_Feedback_Template.md` | FBU/Region 沟通模板 |
| `Flight Upsell Project Hub.md` | FBU 侧权威文档（只读参考） |
| `Flight_Upsell_Trial_Log.md` | Session log |
| `H1_strategist_narrative.md` | H1 review 口头叙事草稿 |
| `H1_self_evaluation.md` | H1 自评正式草稿 |
| `Strategic Portfolio - Global Flight Fare Upsell Optimization.md` | SLT pitch / CV bullet |
| `Strategic Framework - Brand Fare Coverage Optimization.md` | Triage & Trigger 方法论 |

**已迁出（iCloud Drive `Documents/Audit/`）：**
- `audit/` — 航司 audit 数据分析
- `scraper/` — TK/NH/JL 爬虫脚本
