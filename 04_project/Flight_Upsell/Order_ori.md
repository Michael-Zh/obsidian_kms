-- order_ori — AirAsia TH/MY/PH domestic, ori_brand_name version (no brand_fare_mapping CTE)
-- Based on: MZ_202606_quantification_v2
-- Target table: trip-ibu-adhoc.ibu_adhoc_temp.MZ_order_ori_AirAsia
--
-- Changes vs. v2:
--   - brand_en_name CTE removed entirely (airline_brandtier + brand_name removed from output)
--   - ori_brand_name + atpco_brand_name kept as-is from edw_prd_flt_factfltsegment_eng
--   - checkin_bagweight added from edw_ord_flt_order for bag weight distribution analysis
--   - orderdate_d added to output for daily-level analysis
--   - Airline scope: AK / FD / XJ / D7 / Z2 / QZ / KT (all AirAsia entities)
--   - Route scope: TH/MY/PH domestic only (region IN ('th','my','ph') AND Dcountry = Acountry)
--   - Date range: 2026-01-01 to 2026-07-15 (covers VPL launch Jun 17)

CREATE OR REPLACE TABLE `trip-ibu-adhoc.ibu_adhoc_temp.MZ_order_ori_AirAsia`
AS
WITH
  xprod AS (
    SELECT
      o.primaryorderid_fill,
      MAX(IF(x.productname = '手提行李'
               AND x.booktype = 1
               AND x.businesstype_detail = 4,              1, 0)) AS carryon_bundle,
      MAX(IF(x.productname = '行李额'
               AND x.booktype = 1
               AND x.businesstype_detail = 4,              1, 0)) AS checkbag_bundle,
    ...
    WHERE
      DATE(x.orderdate) BETWEEN "2026-01-01" AND "2026-07-15"
      AND x.sequence = 1
    GROUP BY 1
  ),
  ...
  lowest_price_ranked AS (
    ...
    WHERE
      CAST(d AS DATE) BETWEEN "2026-01-01" AND "2026-07-15"
      AND is_trip = 'T'
  ...
  f_textdetail AS (
    SELECT DISTINCT
      ...
      a.sequence,
      a.policytokenno
    FROM `trip-ibu-bi-dw-etl.ibu_bi_dw_source.ods_fltairtickets_fltorderdb_o_flightextdetail` a
    JOIN (
      SELECT DISTINCT orderid
      FROM `trip-ibu-bi-dw-etl.ibu_bi_dw_cdw.edw_ord_flt_order`
      WHERE orderdate_d BETWEEN "2026-01-01" AND "2026-07-15"
        AND is_primaryorder = 1
    ) b ON a.orderid = b.orderid
    WHERE
      DATE(a.d) = DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
      AND DATE(a.datachange_createtime) >= "2025-12-01"
      AND a.sequence = 1
  ),
  ...
  g_other AS (
    ...
    FROM `trip-ibu-bi-dw-etl.ibu_bi_dw_source.ods_fltairtickets_mysql_fltintlpenaltydb_o_other_detail` a
    JOIN (
      SELECT DISTINCT orderid
      FROM `trip-ibu-bi-dw-etl.ibu_bi_dw_cdw.edw_ord_flt_order`
      WHERE orderdate_d BETWEEN "2026-01-01" AND "2026-07-15"
  ...
  e_customer_cancellation AS (
    SELECT DISTINCT
      e.token,
      e.service_fee_type
    FROM `trip-ibu-bi-dw-etl.ibu_bi_dw_source.ods_fltairtickets_mysql_fltintlpenaltydb_o_customer_cancellation` e
    JOIN (
      SELECT DISTINCT orderid
      FROM `trip-ibu-bi-dw-etl.ibu_bi_dw_cdw.edw_ord_flt_order`
      WHERE orderdate_d BETWEEN "2026-01-01" AND "2026-07-15"
        AND is_primaryorder = 1
    ) b ON e.orderid = b.orderid
    WHERE
  ...
  x_BD_ChannelConfigInfo AS (
    SELECT DISTINCT
      ...
      x.market,
      x.channelenname
    FROM `trip-ibu-bi-dw-etl.ibu_bi_dw_source.dim_fltdb_dim_bd_channelconfiginfo` x
  ...
  segment_table AS (
    SELECT DISTINCT
      ...
      a.subchannel,
      -- OD
      a.dportcode, a.dcityid, a.dcitycode, a.dcountryename,
      dct.country_code AS Dcountry_code,
      a.aportcode, a.acityid, a.acitycode, a.acountryename,
      ...
      IF(
        dct.country_code IS NULL OR act.country_code IS NULL OR dimf1.countrycode IS NULL,
        'N',
        IF(
          dct.country_code = dimf1.countrycode AND act.country_code = dimf1.countrycode,
          'D', 'I')
      ) AS RouteType,
      -- Supplier info
      CASE
      ...
      -- Baggage: fare base only (excludes X-product bundle — use this for base fare grouping)
      CASE
        WHEN a.is_free_checkinbagen = 'Y' AND a.is_free_carryonbagen = 'Y'   THEN '4.Fare+carry on+checkin'
        WHEN a.is_free_checkinbagen = 'Y' AND a.is_free_carryonbagen != 'Y'  THEN '3.fare+check in'
        WHEN a.is_free_checkinbagen != 'Y' AND a.is_free_carryonbagen = 'Y'  THEN '2.fare+carry on'
        WHEN a.is_free_checkinbagen != 'Y' AND a.is_free_carryonbagen != 'Y' THEN '1.fare only'
        ELSE 'other'
      END AS baggage_allowance_group,
      -- Baggage: fare base + bundle
      CASE
        WHEN a.is_free_checkinbagen IS NULL AND a.is_free_carryonbagen IS NULL
          AND x.checkbag_bundle IS NULL AND x.carryon_bundle IS NULL          THEN 'other'
        WHEN (a.is_free_checkinbagen = 'Y' OR x.checkbag_bundle = 1)
          AND (a.is_free_carryonbagen = 'Y' OR x.carryon_bundle = 1)         THEN '4.Fare+carry on+checkin'
        WHEN (a.is_free_checkinbagen = 'Y' OR x.checkbag_bundle = 1)         THEN '3.fare+check in'
        WHEN (a.is_free_carryonbagen = 'Y' OR x.carryon_bundle = 1)          THEN '2.fare+carry on'
        ELSE '1.fare only'
      END AS baggage_incl_bundle_group,
      -- Baggage: final resolved field
      CASE
        WHEN a.has_checkin_baggage != 'Y' AND a.has_carryon_baggage != 'Y'   THEN '1.fare_only_final'
        WHEN a.has_checkin_baggage != 'Y' AND a.has_carryon_baggage = 'Y'    THEN '2.fare_carry_on_only_final'
        WHEN a.has_checkin_baggage = 'Y'  AND a.has_carryon_baggage != 'Y'   THEN '3.fare_check_in_only_final'
        WHEN a.has_checkin_baggage = 'Y'  AND a.has_carryon_baggage = 'Y'    THEN '4.fare_both_bag_final'
      END AS baggage_final,
      -- Checkin bag weight — explore distribution and data quality
      a.checkin_bagweight,
      -- Flexibility
      flexible.flexible,
      -- Cabin
      ...
      CASE
        WHEN x.rp_bundle = 1 AND flexible.flexible = '1.Not Flexible'                                THEN '3.Cancel Only'
        WHEN x.rp_bundle = 1 AND flexible.flexible IN ('2.Change Only','3.Cancel Only','4.Flexible') THEN '4.Flexible'
    ...
    LEFT JOIN `trip-ibu-bi-dw-etl.ibu_bi_dw_dim.dim_prd_flt_flightagencyattribute` d
    ...
    LEFT JOIN (
      ...
      WHERE orderdate_d BETWEEN "2026-01-01" AND "2026-07-15"
      GROUP BY orderid
    ) flexible ON a.orderid = flexible.orderid
    LEFT JOIN lowest_price lp ON a.primaryorderid_fill = lp.primary_orderid
    LEFT JOIN xprod x         ON a.primaryorderid_fill = x.primaryorderid_fill
    LEFT JOIN f_textdetail ft  ON a.orderid = ft.orderid
    LEFT JOIN g_other g        ON ft.policytokenno = g.token
    LEFT JOIN e_customer_cancellation e ON ft.policytokenno = e.token
    WHERE
      a.orderdate_d BETWEEN "2026-01-01" AND "2026-07-15"
      AND a.is_primaryorder = 1
      AND a.flightclass = 'I'
      AND a.airline IN ('AK', 'FD', 'XJ', 'D7', 'Z2', 'QZ', 'KT')
      AND b.classname IN ('经济舱', '超级经济舱')
      AND a.subprdtype_pos_primorder NOT IN ('FCN')
      AND a.region IN ('th', 'my', 'ph')
      AND dct.country_code IN ('TH', 'MY', 'PH')
      AND dct.country_code = act.country_code
  )
...
SELECT
  ...
  order_month,
  orderdate_d,
  CONCAT(order_year, '_', LPAD(CAST(order_month AS STRING), 2, '0')) AS year_month,
  -- Market
  chan.market,
  chan.channelenname,
  dportcode, dcitycode, Dcountry_code,
  aportcode, acitycode, Acountry_Code,
  airport_pair, city_pair, country_pair,
  ...
  CASE
    WHEN order_marketing_airline IN (
  ...
  -- Brand name fields (ori = raw from source, atpco = internal mapping)
  atpco_brand_name,
  ori_brand_name,
  -- Baggage dimensions
  checkin_bagweight,
  ...
  COUNT(DISTINCT CASE WHEN is_lowest_price IS NOT NULL      THEN a.primaryorderid_fill END) AS upsell_base_primary_ord_count,
  COUNT(DISTINCT CASE WHEN is_lowest_price = 0              THEN a.primaryorderid_fill END) AS upsell_primary_ord_count,
  -- Void / 24hr counts
  COUNT(DISTINCT CASE WHEN ticketoperationtype = '2'         THEN a.primaryorderid_fill END) AS free_cancel_24h_ord_count,
  COUNT(DISTINCT CASE WHEN ticketoperationtype = '1'         THEN a.primaryorderid_fill END) AS void_ord_count,
  COUNT(DISTINCT CASE WHEN ticketoperationtype IN ('1','2')  THEN a.primaryorderid_fill END) AS free_cancel_or_void_ord_count,
  -- Flexibility bundle counts
  COUNT(DISTINCT CASE WHEN rp_bundle = 1              THEN a.primaryorderid_fill END) AS rp_bundle_ord_count,
  COUNT(DISTINCT CASE WHEN all_flexibility_bundle = 1  THEN a.primaryorderid_fill END) AS all_flexibility_bundle_ord_count
FROM segment_table a
LEFT JOIN x_BD_ChannelConfigInfo chan
  ON  a.source = chan.channel
